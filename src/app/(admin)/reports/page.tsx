...
import { fetchReports, fetchReportDetail } from "@/apis/admin/reports.api";
...
export default function ReportAdminPage() {
  ...
  const handleSelect = useCallback(async (report: ReportItem) => {
    setSelectedReport(report);
    try {
      const detail = await fetchReportDetail(report.type, report.targetId);
      setSelectedReport((prev) => prev?.targetId === report.targetId ? { ...prev, ...detail } : prev);
    } catch (err) {
      console.error("Report detail error:", err);
    }
  }, []);

  const handleProcessed = useCallback((targetId: string, newStatus: string) => {
    setReports((prev) => prev.map((r) => r.targetId === targetId ? { ...r, status: newStatus } : r));
    setSelectedReport((prev) => prev?.targetId === targetId ? { ...prev, status: newStatus } : prev);
  }, []);
  ...
  return (
    <main className="flex min-h-full flex-1 flex-col overflow-hidden bg-[#121212] text-gray-200 font-sans">
      <header className="shrink-0 p-4 pb-3 sm:p-6 sm:pb-4 lg:p-8 lg:pb-4">
        ...
        <p className="text-sm text-gray-400 mt-1">유저들이 신고한 게시글 및 댓글을 모니터링하고 처리합니다.</p>
      </header>
      ...
      <div className="flex shrink-0 flex-col gap-3 border-b border-[#2C2C2C] px-4 py-3 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
        ...
        <label className="flex cursor-pointer items-center gap-2 rounded-md border border-[#2C2C2C] bg-[#1E1E1E] px-3 py-1.5 text-xs hover:border-gray-500">
          <input type="checkbox" checked={filterMinFive} onChange={(e) => setFilterMinFive(e.target.checked)} className="accent-orange-500" />
      ...
      <div className="flex flex-1 flex-col gap-4 overflow-y-auto px-4 py-4 sm:px-6 sm:py-6 lg:flex-row lg:gap-6 lg:overflow-hidden lg:px-8">
        {isLoading ? (
          ...
          <>
            <ReportTable reports={filteredReports} selectedReport={selectedReport} onSelect={handleSelect} />
            <ReportDetailPanel selectedReport={selectedReport} onProcessed={handleProcessed} />
          </>
        )}
      </div>
    </main>
